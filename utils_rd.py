...
def evaluate(model, P_tensor, P_time_tensor, P_static_tensor, P_delta_t_tensor, batch_size=100, n_classes=2, static=None):
    model.eval()
    ...
    for i in range(n_batches):
        ...
        middleoutput = model.forward(P.permute(1, 0, 2), P_delta_t.cuda(), Pstatic.cuda(), lengths)
        out[start:start + batch_size] = middleoutput.detach().cpu()
        start += batch_size
    if rem > 0:
        P = P_tensor[:, start:start + rem, :]
        Ptime = P_time_tensor[:, start:start + rem]
        P_delta_t = P_delta_t_tensor[start:start + rem, :]

        if P_static_tensor is not None:
            Pstatic = P_static_tensor[start:start + batch_size]
        lengths = torch.sum(Ptime > 0, dim=0) + 1
        whatever = model.forward(P.permute(1, 0, 2), P_delta_t.cuda(), Pstatic.cuda(), lengths)
        out[start:start + rem] = whatever.detach().cpu()
    return out

def evaluate_ASTGCN(model, P_tensor, P_time_tensor, P_static_tensor, batch_size=100, n_classes=2, static=None):
    model.eval()
    P_tensor = P_tensor.cuda()
    P_time_tensor = P_time_tensor.cuda()
    if static is None:
    ...
    for i in range(n_batches):
        ...
        if P_static_tensor is not None:
            Pstatic = P_static_tensor[start:start + batch_size]
        lengths = torch.sum(Ptime > 0, dim=0) + 1
        middleoutput = model.forward(P.permute(1, 0, 2))
        out[start:start + batch_size] = middleoutput.detach().cpu()
        start += batch_size
    if rem > 0:
        P = P_tensor[:, start:start + rem, :]
        ...
        if P_static_tensor is not None:
            Pstatic = P_static_tensor[start:start + batch_size]
        lengths = torch.sum(Ptime > 0, dim=0) + 1
        whatever = model.forward(P.permute(1, 0, 2))
        out[start:start + rem] = whatever.detach().cpu()
    return out
def evaluate_standard(model, P_tensor, P_time_tensor, P_static_tensor, batch_size=100, n_classes=2, static=None):
    P_tensor = P_tensor.permute(1, 0, 2).cuda()
    P_time_tensor = P_time_tensor.cuda()
    if static is None:
        P_static_tensor = None
    else:
        P_static_tensor = P_static_tensor.cuda()
    n_batches = P_tensor.shape[0] // batch_size
    out = []
    for i in range(n_batches + 1):
        lengths = torch.sum(P_time_tensor > 0, dim=0) + 1
        out_i= model.forward(P_tensor[i * batch_size: min((i+1) * batch_size, P_tensor.shape[0])], lengths[i * batch_size: min((i+1) * batch_size, lengths.shape[0])])
        out.append(out_i)
    out = torch.cat([i for i in out])
    return out

def evaluate_standrad_ASTGCN(model, P_tensor, P_time_tensor, P_static_tensor, batch_size=100, n_classes=2, static=None):
    P_tensor = P_tensor.permute(1, 0, 2).cuda()
    P_time_tensor = P_time_tensor.cuda()
    if static is None:
        P_static_tensor = None
    ...
    for i in range(n_batches + 1):
        lengths = torch.sum(P_time_tensor > 0, dim=0) + 1
        out_i= model.forward(P_tensor[i * batch_size: min((i+1) * batch_size, P_tensor.shape[0])])
        out.append(out_i)
    out = torch.cat([i for i in out])
    return out
