...
class single_channel_interp(Layer):
    ...
    def call(self, x, reconstruction=False):
        ...
        w = K.logsumexp(-alpha*norm + K.log(tf.clip_by_value(mask,1e-8,1.0)), axis=2)
        w1 = K.tile(w[:, :, None, :], (1, 1, self.time_stamp, 1))
        w1 = K.exp(-alpha*norm + K.log(tf.clip_by_value(mask,1e-8,1.0)) - w1)
        y = K.sum(w1*x_t, axis=2)
        if reconstruction:
            rep1 = tf.concat([y, w], 1)
        else:
            w_t = K.logsumexp(-10.0*alpha*norm + K.log(tf.clip_by_value(mask,1e-8,1.0)),
                              axis=2)  # kappa = 10
            w_t = K.tile(w_t[:, :, None, :], (1, 1, self.time_stamp, 1))
            w_t = K.exp(-10.0*alpha*norm + K.log(tf.clip_by_value(mask,1e-8,1.0)) - w_t)
