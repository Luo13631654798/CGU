    ...
    for k in range(0, n_splits):
        ...
        if dataset == 'P12' or dataset == 'P19' or dataset == 'physionet':
            ...
            if args.missingtype == 'time':
                Ptrain_tensor, Ptrain_static_tensor, Ptrain_delta_t_tensor, Ptrain_length_tensor, Ptrain_time_tensor, ytrain_tensor \
                    = tensorize_normalize_misssing(Ptrain, ytrain, mf, stdf, ms, ss, missingtype=args.missingtype,
                ...
                Pval_tensor, Pval_static_tensor, Pval_delta_t_tensor, Pval_length_tensor, Pval_time_tensor, yval_tensor \
                    = tensorize_normalize_misssing(Pval, yval, mf, stdf, ms, ss, missingtype=args.missingtype,
                                                   missingratio=missing_ratio)
                Ptest_tensor, Ptest_static_tensor, Ptest_delta_t_tensor, Ptest_length_tensor, Ptest_time_tensor, ytest_tensor \
                    = tensorize_normalize_misssing(Ptest, ytest, mf, stdf, ms, ss, missingtype=args.missingtype,
                                                   missingratio=missing_ratio)
            ...
            if args.missingtype == 'time':
                Ptrain_tensor, Ptrain_static_tensor, Ptrain_delta_t_tensor, Ptrain_length_tensor, Ptrain_time_tensor, ytrain_tensor \
                    = tensorize_normalize_other_missing(Ptrain, ytrain, mf, stdf, missingtype=args.missingtype,
                                                        missingratio=missing_ratio, idx=idx)
                Pval_tensor, Pval_static_tensor, Pval_delta_t_tensor, Pval_length_tensor, Pval_time_tensor, yval_tensor \
                    = tensorize_normalize_other_missing(Pval, yval, mf, stdf, missingtype=args.missingtype,
                                                        missingratio=missing_ratio, idx=idx)
                Ptest_tensor, Ptest_static_tensor, Ptest_delta_t_tensor, Ptest_length_tensor, Ptest_time_tensor, ytest_tensor \
                    = tensorize_normalize_other_missing(Ptest, ytest, mf, stdf, missingtype=args.missingtype,
                                                        missingratio=missing_ratio, idx=idx)
            elif args.missingtype == 'variable':
                Ptrain_tensor, Ptrain_static_tensor, Ptrain_delta_t_tensor, Ptrain_length_tensor, Ptrain_time_tensor, ytrain_tensor \
                    = tensorize_normalize_other_missing(Ptrain, ytrain, mf, stdf, missingtype=args.missingtype,
        ...
        if baseline == 'SEFT':
            ...
            model = SEFT(variables_num, variables_num, nhead, nhid, nlayers, dropout, timestamp_num, d_static, MAX, 0.5, aggreg,
                         n_class, static=False if d_static == 0 else True).to(device)
        elif baseline == 'MTGNN':
            ...
            x_mean = torch.where(torch.isnan(x_mean), torch.full_like(x_mean, 0), x_mean)
            ...
            for n in range(n_batches):
                if dataset == 'P12' or dataset == 'P19' or dataset == 'physionet' or dataset == 'mimic3':
                    ...
                    P, Ptime, Pdelta_t, Plength, Pstatic, y = Ptrain_tensor[:, idx, :].cuda(), Ptrain_time_tensor[:, idx].cuda(),\
                        ...
                        Ptrain_static_tensor[idx].cuda() if d_static != 0 else None, ytrain_tensor[idx].cuda()
                elif dataset == 'PAM':
            ...
            with torch.no_grad():
                    ...
                    for n in range(n_batches):
                        if dataset == 'P12' or dataset == 'P19' or dataset == 'physionet' or dataset == 'mimic3':
                            out_val_tensors.append(evaluate_DGM2(model, Pval_tensor[:, n * batch_size: (n + 1) * batch_size, :-1],
                                               Pval_static_tensor[n * batch_size: (n + 1) * batch_size, :] if d_static != 0 else None,
                                                                  static=None if d_static == 0 else 1))
                        elif dataset == 'PAM':
                            out_val_tensors.append(evaluate_DGM2(model, Pval_tensor[:, n * batch_size: (n + 1) * batch_size, :-1],
                                                                  None, static=None if d_static == 0 else 1))
                ...
                if dataset == 'P12' or dataset == 'P19' or dataset == 'physionet' or dataset == 'mimic3':
                    ...
                    print(
                        ...
                        (epoch, val_loss.item(), acc_val * 100, aupr_val * 100, auc_val * 100, precision * 100,
                         recall * 100, F1 * 100))
        ...
        with torch.no_grad():
            ...
            y_test = ytest.copy()
            if np.isinf(denoms).any():
                probs = np.array(torch.sigmoid(torch.cat(out_test_tensors, dim=0).detach().cpu()))
            else:
